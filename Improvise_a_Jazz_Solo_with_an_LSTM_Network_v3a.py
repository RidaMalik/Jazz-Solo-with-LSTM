...
def djmodel(Tx, n_a, n_values):
    ...
    Arguments:
    Tx -- length of the sequence in a corpus <Gema> 30
    n_a -- the number of activations used in our model <Gema> 64
    n_values -- number of unique values in the music data <Gema> 78
    ...
    X = Input(shape=(Tx, n_values)) # <Gema> Input() is used to instantiate a Keras tensor. Using this layer as the first layer in a model.
    # <Gema> shape: A shape tuple (integer), not including the batch size. For instance, shape=(32,) indicates that the expected input will be batches of 32-dimensional vectors.
    ...
    outputs = [] # Create an empty list "outputs" to save the outputs of the LSTM Cell at every time step.
    ...
    # <Gema> This creates a model that includes the Input layer and Tx Dense layers
